...
# takes about 46 seconds instead

system.time(
  long_test_T <- future_apply_Bootstrap_SE_nonspecific(test1$sim_data.L, var.component = "TRUE", R = 100)
)
...
# time for 4 application withs 4 cores, 100 samples and 100 bootstraps: 420 sconds / 7 minutes
# time for 1 application with 4 cores, 100 samples and 1000 bootstraps: 4094 sconds / 68.23 minutes
# time for 1 application with 4 cores, 100 samples and 100 bootstraps:  246 sconds / 4.11 minutes

# time for 4 applications with 4 cores, 100 samples and 100 bootstraps: 9.3 seconds (IF: calc Cronbach's Alpha by myself)
# time for 80 applications with 4 cores, 100 samples and 100 bootstraps: 130 seconds (IF: calc Cronbach's Alpha by myself)
# time for 80 applications with 1 core, 100 samples and 100 bootstraps: 300 seconds
...
system.time(
  Large_Sim_Data <- future_lapply(1:nrow(all_conditions[1:160,]), future.seed = TRUE, FUN = function(x){
    ...
    b.data_T <- apply_Bootstrap_SE_nonspecific(it.simdata$sim_data.L, var.component = "TRUE", R = 1000)
    b.data_E <- apply_Bootstrap_SE_nonspecific(it.simdata$sim_data.L, var.component = "ERROR", R = 1000)
...
Large_Sim_Data <- readRDS(file = here("Notes/Sim_8000_conditions.RData"))
